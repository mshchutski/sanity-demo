    ...
    <footer className="dark bg-zinc-900 mt-auto px-8 pt-20">
      <div className="w-full max-w-screen-xl mx-auto py-4 md:py-8">
        <div className="sm:flex sm:items-center sm:justify-between">
          ...
          <ul className="flex flex-wrap items-center mb-6 text-sm font-medium text-gray-500 sm:mb-0 dark:text-gray-400">
            {NAV_LINKS.map(({slug, title}, index) => {
              ...
              return (
                <li key={index}>
                  <Link href={slug} className={`hover:underline me-4 md:me-6 hover:text-white ${active}`}>{title}</Link>
                </li>
              )
            })}
          </ul>
        ...
        <span className="block text-sm text-gray-500 sm:text-center dark:text-gray-400">© 2024 <a
          href="/" className="hover:underline hover:text-white">Testtest™</a>. All Rights Reserved.</span>
      </div>
    </footer>
  )
}
